...
let revealedLetters = document.querySelectorAll('.letter.show')
...
class Game {
	...
	gameOver(outcome) {
		...
		document.getElementById('game-over-message').textContent = outcome === 'won' ? 'Congratulations, You Won!' : 'Sorry, You Lost. Try Again!'
	}
	checkForWin() {
	...
	addKeyListeners() {
		keys.forEach((key) => {
			key.addEventListener('click', () => {
				// Handle interaction for the clicked key
				const isWrong = !this.activePhrase.checkLetter(key.textContent.toLowerCase())
				if (isWrong) {
					key.classList.add('wrong')
					this.removeLife()
					append(key).disabled = true
				}
			})
		})
	}
